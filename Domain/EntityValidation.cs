...
public static class EntityValidation
{
    ...
    public static IReadOnlyList<string> Validate([NotNull] this Entity value)
    {
        ArgumentNullException.ThrowIfNull(value);

        var errors = new List<string>();

        // Validate required string properties using pattern matching
        ...
        // Validate collections - these should never be null due to collection initialization in Entity
        ...
        if (value.Attributes.Count > 100)
        {
            errors.Add("Entity cannot have more than 100 attributes.");
        }

        if (value.Interfaces.Count > 50)
        {
            errors.Add("Entity cannot implement more than 50 interfaces.");
        }

        // Validate dates - CreatedAt should not be default and should not be in the future
        ...
        // UpdatedAt should not be default and should not be in the future
        ...
        {
            errors.Add("Entity UpdatedAt date cannot be earlier than CreatedAt date.");
        }

        // Validate AccessModifier using pattern matching
        if (value.AccessModifier is not (AccessModifier.Public or AccessModifier.Private or
                                         AccessModifier.Protected or AccessModifier.Internal or
                                         AccessModifier.ProtectedInternal))
        {
            errors.Add("Entity AccessModifier has an invalid value.");
        }

        // Validate property names for duplicates using collection methods
        ...
        // Validate each property using LINQ for cleaner code
        errors.AddRange(value.Properties
            .SelectMany(property => property.Validate())
            .ToList());
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="value"/> is null.</exception>
    public static bool IsValid([NotNull] this Entity value)
    {
        ArgumentNullException.ThrowIfNull(value);
    ...
    public static void EnsureValid([NotNull] this Entity value)
    {
        ArgumentNullException.ThrowIfNull(value);
